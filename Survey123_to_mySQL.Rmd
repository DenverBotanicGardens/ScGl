...
```{r}
## x and y are for the site, not the plant
...
## These don't have X and Y of the tags. Need the data upload csv from the DBG Database for those
plants2024 %>%
  left_join(demo2024, by = c("ParentGlobalID" = "GlobalID")) %>%
  left_join(sitesTransID, by = c("transect" = "TransectID")) %>%
...
## Should be no rows with no Tag but a height
plants2024 %>%
  left_join(demo2024, by = c("ParentGlobalID" = "GlobalID")) %>%
  left_join(sitesTransID, by = c("transect" = "TransectID")) %>%
