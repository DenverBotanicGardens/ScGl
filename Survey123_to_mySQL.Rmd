...
dataWQAQC_v2 %>%
  filter(grepl(paste(c("tag ", "Tag "), collapse = "|"),
              Comments.x)) %>% 
  # mutate(Comments = toupper(Comments.x)) %>%
  ## This isn't helpful
  # mutate(NewX = regmatches(Comments, gregexpr("(?<=X).*", Comments, perl = TRUE))) %>% 
  # mutate(NewY = regmatches(Comments, gregexpr("(?<=Y).*", Comments, perl = TRUE)))
  mutate(NewIndivdLocation)
