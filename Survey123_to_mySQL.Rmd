...
## Added distinct to the creation of data2QAQC
## Some duplicates?! why? There are 197 when duplicates removed, there are 696 with duplicates; maybe some duplicates in the database
data2QAQC %>%
  filter(is.na(ScGl_tag_id)) %>%
  distinct(Tag, Height, Width, Flowering, .keep_all = TRUE) %>%
  summarise(n = n())  ## to check the number, much more than the 15 where we recorded X and Y, but if new plant off existing tag, then no XY
...
## Find the duplicates and check if needed changes in database; 705 tags duplicated! What?!?!
dups2024 <- data2QAQC %>%
  group_by(Tag, Height, Width) %>%
  filter(n() > 1) 
dups2024 %>%
  distinct(Tag, .keep_all = TRUE)
head(as.data.frame(dups2024))

# TagID 189 died in 2019, still dead, why duplicated? 

## I guess just get rid of duplicates for entering data


  


## Where is ScGl_tag_id coming from? plants2024 it is a chr; 
...
Duplicates:
ScGl_tag_id 2254 and 2476 for 628.01; 2476 no more data with it
