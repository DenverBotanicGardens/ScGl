...
annualdata2QC <- plants2024 %>%
  ...
  mutate(ScGl_tag_id = as.numeric(as.character(ScGl_tag_id))) %>%
...
data2vlookup <- plants2024 %>%
  left_join(demo2024, by = c("ParentGlobalID" = "GlobalID")) %>%
  left_join(sitesTransID, by = c("transect" = "TransectID")) %>%
  # mutate(ScGl_tag_id = as.numeric(ScGl_tag_id)) %>%
  full_join(df2enter, by = c("ScGl_tag_id"="TagIdignoreforupload")) %>%
  select(c(Tag.x,Tag.y,Height:ScGl_tag_id, site:transect, Site:IndividualPlantLocation)) %>%
  arrange(Site,transect,Tag.x) %>%
  filter(!(is.na(Height) & Tag.x == "" & is.na(X..Minis)))
