...
cor.level <- .5 
exclude86.current <- apply(data86.current[abs(data86.current$Freq)>cor.level&
                                            data86.current$Freq<1,], 1, function(x){
  #Which varaible explains less yearly variation between the two correlated
  rsqs$var[rsqs$r2 == min(rsqs$r2[rsqs$var==x[1]],rsqs$r2[rsqs$var==x[2]])]
})


#combins <- combn(colnames(cor.table86.current),2,FUN = function(x) {paste(x,collapse="_")})
...
tomatch <- c("Year","Month","Site","Prev12",levels(exclude86.current))
...
# Just use everything? 
forpca86.current <- cc.season[,c(grep(paste(tomatch,collapse="|"),
                            names(cc.season), value=TRUE, invert = TRUE))]
