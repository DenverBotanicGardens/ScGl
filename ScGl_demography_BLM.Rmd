...
ggsave(paste(userpath, "Denver Botanic Gardens/Conservation - General/AllProjectsBySpecies/Sclerocactus-glaucus/Sclerocactus-glaucus_AnnualReports/2021_Sclerocactus-glaucus_AnnualReport/",currentYr,"Fig6.jpg", sep=""),
...
## Should be binomial
lm1 <- glm(Prepro~avgVol,family = "binomial", sg_sum, weights = X)
...
lm.list <- list(lm1,lm2, lm10,lm11,lm12,lm13)
lm.names <- as.character(unlist(lapply(lm.list,formula)))
(lm.results <- aictab(lm.list, modnames=lm.names))
...
# more percent reproductive with avg width
summary(lm1)
effectsize::standardize(lm1) %>% summary()
sjPlot::plot_model(lm1, type = "eff", show.data = T) 
## meansd : mean value +/- one SD
# "int" for continuous variables the min and max are chosen as grouping levels
set_theme(base = theme_bw())
p1 <- sjPlot::plot_model(lm1, type = "int", mdrt.values = "meansd")
p1[[11]] + 
  scale_colour_discrete("Min Temp")+
  plot_annotation(title = "Probability of Reproduction",
                  tag_levels = "a")+
  ggtitle("")+
  xlab("")
...
all1 <- p1[[11]][[1]]+
  ylab("% reproductive")+
  labs(title = "", subtitle = "Annual precipitation = 271.53")+
  ggtitle("a)")
all2 <- p1[[11]][[2]]+
  ylab("% reproductive")+
  labs(title = "", subtitle = "Annual precipitation = 182.47")+
  ggtitle("b)")
all3 <- p1[[11]][[3]]+
  ylab("% reproductive")+
  xlab("average width (mm)")+
  labs(title = "", subtitle = "Annual precipitation = 360.59")+
  scale_color_discrete("Min temp")+
  ggtitle("c)")
  
ggsave(paste(userpath, "Denver Botanic Gardens/Conservation - General/AllProjectsBySpecies/Sclerocactus-glaucus/Sclerocactus-glaucus_AnnualReports/2021_Sclerocactus-glaucus_AnnualReport/",currentYr,"Fig4.jpg", sep=""),
       all1 / all2 / all3,
width = 150, height = 170, units = 'mm', dpi=300)

ggsave(paste(userpath, "Denver Botanic Gardens/Conservation - General/AllProjectsBySpecies/Sclerocactus-glaucus/Sclerocactus-glaucus_AnnualReports/2021_Sclerocactus-glaucus_AnnualReport/",currentYr,"Fig4_tmin_avgW.jpg", sep=""),
  p1[[1]] +
    scale_colour_discrete("Min Temp")+
    ggtitle("")+
    ylab("% reproductive")+
...
# The effect of min and max temperatures when ppt is held constant
p1[[3]] +
  scale_colour_discrete("Max Temp")+
  ggtitle("")+
  ylab("Probability of Reproduction")+
...
p1[[7]]
  ...
  # theme_sjplot(axis_title.X = "average width")
sjPlot::plot_model(lm11, type = "est", show.values = T, transform = NULL)
sjPlot::tab_model(lm11, transform = NULL) # estimates on linear scale

sg_sum$annual.tminbin <- cut_width(sg_sum$data.tmin, 5) # ((max(sg_sum$ppt.winter)+0.01)-(min(sg_sum$ppt.winter)-0.01))/2 )

ggplot(sg_sum, aes( data.tmin, Prepro, colour = avgW))+
  geom_point()+
  stat_smooth(method = "glm", method.args = list(family = "binomial"))+
  theme_bw()

ggplot(sg_sum, aes( avgW, Prepro,colour = data.tmin))+
  geom_point()+
  stat_smooth(method = "glm", method.args = list(family = "binomial")) +
  facet_wrap(~annual.tminbin)
```
...
ggsave(paste(userpath, "Denver Botanic Gardens/Conservation - General/AllProjectsBySpecies/Sclerocactus-glaucus/Sclerocactus-glaucus_AnnualReports/2021_Sclerocactus-glaucus_AnnualReport/",currentYr,"Fig3.jpg", sep=""),
Fig3a/Fig3b/Fig3c,
...
ggplot(sg.pc.lambda, aes(ppt.fall, ySpecies))+
  geom_point()+
  stat_smooth(method="lm")
...
ggsave(filename = paste(userpath, "Denver Botanic Gardens/Conservation - General/AllProjectsBySpecies/Sclerocactus-glaucus/Sclerocactus-glaucus_AnnualReports/2021_Sclerocactus-glaucus_AnnualReport/",currentYr,"Fig5a_b.jpg", sep=""),
ggheight/ggwidth,
