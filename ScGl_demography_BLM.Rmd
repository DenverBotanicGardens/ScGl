...
sg.JS <- sg.newold %>%
  ...
  ############ UPDATE ##################
  unite(ch, Year2008:Year2024, sep="") %>%
  filter(grepl("1",ch))
...
run.scgl <- function(){
  ...
  p.dot = list(formula =  ~ 1, fixed = list(index = p.indicesLinkBarkRan, value = p.valuesLinkBarkRan))
...
save(scgl.LinkBarkRan, file = paste(userpath, "Denver Botanic Gardens/Conservation - General/AllProjectsBySpecies/Sclerocactus-glaucus/Sclerocactus-glaucus_AnnualReports/",currentYr,"_Sclerocactus-glaucus_AnnualReport/",currentYr,"LinkBarkRan.Rdata", sep=""))
...
run.scgl <- function(){
  ...
  Phi.Time = list(formula =  ~ Time) ## Trend?
  Phi.Plot = list(formula =  ~ Site)  ## so each plot is offset
  Phi.pptfall = list(formula =  ~ ppt_fall)
  Phi.pptspring = list(formula =  ~ ppt_spring)
  Phi.pptsummer = list(formula =  ~ ppt_summer)
  Phi.pptwinter = list(formula =  ~ ppt_winter)
  Phi.tmaxfall = list(formula =  ~ tmax_fall)
  Phi.tmaxspring = list(formula =  ~ tmax_spring)
  Phi.tmaxsummer = list(formula =  ~ tmax_summer)
  Phi.tmaxwinter = list(formula =  ~ tmax_winter)
  Phi.tminfall = list(formula =  ~ tmin_fall)
  Phi.tminspring = list(formula =  ~ tmin_spring)
  Phi.tminsummer = list(formula =  ~ tmin_summer)
  Phi.tminwinter = list(formula =  ~ tmin_winter)
  ...
  f.Plot = list(formula = ~ Site)
  f.species = list(formula = ~ Species)
  f.pptfall = list(formula =  ~ ppt_fall)
  f.pptspring = list(formula =  ~ ppt_spring)
  f.pptsummer = list(formula =  ~ ppt_summer)
  f.pptwinter = list(formula =  ~ ppt_winter)
  f.tmaxfall = list(formula =  ~ tmax_fall)
  f.tmaxspring = list(formula =  ~ tmax_spring)
  f.tmaxsummer = list(formula =  ~ tmax_summer)
  f.tmaxwinter = list(formula =  ~ tmax_winter)
  f.tminfall = list(formula =  ~ tmin_fall)
  f.tminspring = list(formula =  ~ tmin_spring)
  f.tminsummer = list(formula =  ~ tmin_summer)
  f.tminwinter = list(formula =  ~ tmin_winter)
...
scgl.models$model.table

Phi.table <- get.real(scgl.models$Phi.Plot.p.dot.f.time, "Phi", se = TRUE)
...
f.table <- get.real(scgl.models$Phi.Plot.p.dot.f.time, "f", se = TRUE)
...
## Lambda == Phi + f
Phi.table %>%
  left_join(f.table, by = c(#"all.diff.index","par.index",
    "fixed","note","group","age","time","Age","Time","Site")) %>%
  mutate(Lambda = estimate.x + estimate.y) %>%
group_by(Time, Site) %>%
  summarise(estimates = mean(Lambda)) %>%
  filter(estimates != 0) %>%
...
nrow(scgl.models$Phi.Plot.p.dot.f.time$results$beta)
scgl.models$Phi.Plot.p.dot.f.time$results$derived
