...
Don't run this, use it from Rmisc instead
```{r}
...
Make sure delete provisional from tmax, tmin, precip...
...
colexclude <- c(1:2)
while(max(data86.current$Freq[abs(data86.current$Freq)>cor.level&
                              data86.current$Freq<1])>0.5){
  cor86current <- data86.current[abs(data86.current$Freq)>cor.level&
                                            data86.current$Freq<1,]
  #order by most correlated
  cor86current <- cor86current[with(cor86current, order(-abs(Freq))),]
  exclude <- rsqs$var[rsqs$r2 == min(rsqs$r2[rsqs$var==as.character(cor86current[1,1])],
                                     rsqs$r2[rsqs$var==as.character(cor86current[1,2])])]
  #need to iteratively get rid of exclude
  colexclude <- c(colexclude,grep(as.character(exclude), names(cc.season)))
  cor.table86.current<-cor(cc.season[,-colexclude]) 
  data86.current <- as.data.frame(as.table(cor.table86.current))
}
...
while(max(data.r$Freq[abs(data.r$Freq)>cor.level&
                      data.r$Freq<1])>0.5){
  cor08 <- data.r[abs(data.r$Freq)>cor.level & data.r$Freq<1,]
  #order by most correlated
  cor08 <- cor08[with(cor08, order(-abs(Freq))),]
  exclude08 <- rsqs$var[rsqs$r2 == min(rsqs$r2[rsqs$var==as.character(cor08[1,1])],
                                       rsqs$r2[rsqs$var==as.character(cor08[1,2])])]
  #need to iteratively get rid of exclude
  colexclude08 <- c(colexclude08,grep(as.character(exclude08), names(recent08current)))
  cor.table08<-cor(recent08current[,-colexclude08]) 
  data.r <- as.data.frame(as.table(cor.table08))
}

#Variables kept for 2007 May - current year
names(recent08current[,-colexclude08])
...
write.table(Table3b[!duplicated(Table3b$Freq),], "clipboard", sep="\t", row.names=FALSE)
...
Reults of the second PCA 
```{r}
ggplot(sg.pc.r, aes(tmin.fall,tmax.fall,colour=NS))+
  geom_point()+
  stat_ellipse()

ggplot(sg.pc.r, aes(ppt.winter,ppt.spring,colour=NS))+
  geom_point()+
  stat_ellipse()
```
...
step1 <- as.formula(paste("ySpecies ~(",
                          paste(names(sg.pc.lambda)[10:21],
                                collapse="+"),")^2"))

# Stepwize is so dependant on order of putting in, should do AIC intsead thinking through predictors first
lm4step <- do.call("lm", list(step1, quote(sg.pc.lambda)))
summary(step(lm4step))
...
#Don't do all interactions
steplinear <- as.formula(paste("ySpecies ~",
                          paste(names(sg.pc.lambda)[10:21],
                                collapse="+")))

lm4steplinear <- do.call("lm", list(steplinear, quote(sg.pc.lambda)))
summary(step(lm4steplinear))
...
stepComps <- as.formula(paste("ySpecies ~",
                          paste(names(sg.pc.lambda)[3:4],
                                collapse="+")))

lm4stepComps <- do.call("lm", list(stepComps, quote(sg.pc.lambda)))
summary(lm4stepComps)                   
```
