...
## Only 61 rows missing, all from Powerline (old), tags 38, 39, 43, 51, 58, 183, and 796
sg_annual %>%
  filter(is.na(Value_ppt)) %>%
  group_by(Site, Tag, Transect) %>%
  summarise(n = n())

table(sg_annual$Site, sg_annual$Year)
sg_annual %>%
  filter(SiteSame == "Powerline" & Tag == 38)
## GO back up and remove these from sg 
...
sg_season %>%
  filter(is.na(tmin_winter))  ###AAAAh, now have > 8700 rows with no climate data!!!
...
lm.list <- list(lm1, lm2, lm3, lm4, lm8, lm7, lm10) # lm6 lm9, 
lm.names <- as.character(unlist(lapply(lm.list,formula)))
(lm.results <- aictab(lm.list, modnames=lm.names))
#evidence ratio 
for(i in 2:length(lm.list)){
...
sg_annual %>%
  ...
  mutate(SizeClass = cut(Width.cm., breaks = c(0, 0.5, 3.2, max(Width.cm.)) )) %>%
  # filter(SizeClass == 2) %>%
ggplot(aes(scalevalue_tmin, Fl, colour = NS))+
  geom_point(pch="|") +
  geom_smooth(method = "glm", 
