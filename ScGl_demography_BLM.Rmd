...
atwell.old <- sg[intersect(grep("Atwell",sg$Site),which(atwell$Y.coord.m.<30)),] 
...
write.table(sg_sum[sg_sum$Year == 2016,], "clipboard", 
            sep="\t", row.names=FALSE)
#Report Tables_20XX
```
...
```{r}
scgl.temps <- rbind(do.call(rbind, m.tmin), 
                    do.call(rbind, m.tmax))
scgl.temps$Year <- substr(scgl.temps$date, 25,28)
scgl.temps$Month <- substr(scgl.temps$date, 29,30)

scgl.ppt <- do.call(rbind, m.ppt)
scgl.ppt$Year <- substr(scgl.ppt$date, 24,27)
scgl.ppt$Month <- substr(scgl.ppt$date, 28,29)

scgl.climate <- rbind(scgl.temps,scgl.ppt)
...
data.name <- sapply(scgl.climate$date, function(x) {
  unlist(strsplit(x, split="_", fixed=TRUE))[[2]]
  })
head(data.name)
scgl.climate$Variable <- data.name

str(scgl.climate)

#long to wide
library(reshape)
melt.scgl <- melt(scgl.climate[,-2], id = c("Site","Year","Month","Variable"))
head(melt.scgl)
```
