...
tablesummaryout <- sg_sum %>%
  ...
  dplyr::summarise(area = sum(area), TotInd = sum(X), IndSqM = round(sum(X)/area,2), 
         ...
         AvgHeight = paste(round(mean(avgH),2), " (", round(mean(sdH),2), ")", sep=""),
         AvgWidth = paste(round(mean(avgW),2), " (", round(mean(sdW),2), ")", sep = ""),
         Brs = sum(Brs), PercBr = paste0(round(100*sum(Brs)/sum(X), 2), "%")) %>%
  dplyr::select(Group, Year, area, TotInd, IndSqM, PercRepro, AvgHeight, AvgWidth, Brs, PercBr)
...
sizesummary <- bind_rows(atwell.old, atwell.new, sg) %>%
  ...
  group_by(Group, Year) %>%
  dplyr::summarise(AvgHeight = paste(round(mean(Height.cm., na.rm=TRUE),2), 
                                     " (", round(sd(Height.cm., na.rm=TRUE),2), ")", sep=""),
                   AvgWidth = paste(round(mean(Width.cm., na.rm=TRUE),2), 
                                    " (", round(sd(Width.cm., na.rm=TRUE),2), ")", sep = "")) %>%
  dplyr::select(Group, Year, AvgHeight, AvgWidth)
...
tableoutNS <- sg_sum %>%
  ...
  mutate(TotInd = X, IndSqM = round(X/area,2), PercRepro = paste0(round(100*Rep/X, 1), "%"),
         ...
         AvgWidth = paste(round(avgW,2), " (", round(sdW,2), ")", sep = "")
         PercBr = paste0(round(100*Brs/X, 2), "%")) %>%
  dplyr::select(Year,NS,Site, area, TotInd, IndSqM, Rep, PercRepro, AvgHeight, AvgWidth, Brs, PercBr) %>%
...
tablesummaryoutNS <- sg_sum %>%
  ...
  dplyr::summarise(area = sum(area), TotInd = sum(X), IndSqM = round(sum(X)/area,2), 
         ...
         AvgWidth = paste(round(mean(avgW),2), " (", round(mean(sdW),2), ")", sep = ""),
         Brs = sum(Brs), PercBr = paste0(round(100*sum(Brs)/sum(X), 2), "%")) %>%
  dplyr::select(NS, Year, area, TotInd, IndSqM, Rep, PercRepro, AvgHeight, AvgWidth, Brs, PercBr)
...
sizesummaryNS <- bind_rows(atwell.new, sg) %>%
  ...
  dplyr::summarise(# AvgHeight = paste(round(mean(Height.cm., na.rm=TRUE),2), 
                   ...
                   # AvgWidth = paste(round(mean(Width.cm., na.rm=TRUE),2), 
                                    # " (", round(sd(Width.cm., na.rm=TRUE),2), ")", sep = ""),
                   MedianWidth = paste(round( exp(mean(log(Width.cm.))),2), " (",
                                        round( exp(mean(log(Width.cm.))+ sd(log(Width.cm.))),2),  ")", sep = ""),
                   HDI95_width = paste("(",round(hdi(Width.cm., credMass = 0.95)[1],2),
                               "-",round(hdi(Width.cm., credMass = 0.95)[2],2), "cm)", sep = ""),
                   MedianHeight = paste(round( exp(mean(log(Height.cm.))),2), " (",
                                        round( exp(mean(log(Height.cm.))+ sd(log(Height.cm.))),2),  ")", sep = ""),
                   HDI95_height = paste("(",round(hdi(Height.cm., credMass = 0.95)[1],2),
                               "-",round(hdi(Height.cm., credMass = 0.95)[2],2), "cm)", sep = ""))
...
# write.table(poststand, file = paste(userpath, "Denver Botanic Gardens/Conservation - General/AllProjectsBySpecies/Sclerocactus-glaucus/Sclerocactus-glaucus_AnnualReports/",currentYr,"_Sclerocactus-glaucus_AnnualReport/Table2_poststand_correction.csv", sep=""), sep=",", row.names=FALSE, col.names = TRUE)
# 
# 
# write.table(prestand, file = paste(userpath, "Denver Botanic Gardens/Conservation - General/AllProjectsBySpecies/Sclerocactus-glaucus/Sclerocactus-glaucus_AnnualReports/",currentYr,"_Sclerocactus-glaucus_AnnualReport/Table2_prestand_correction.csv", sep=""),
...
brflowered <- ggplot(sg_season_scaled[!is.na(sg_season_scaled$Fl),], 
...
ggplot(sg_season_scaled[!is.na(sg_season_scaled$Fl),], 
       aes(ppt_winter, as.numeric(Fl), colour = Br))+
  geom_point()+
  facet_wrap(~NS)+
  geom_smooth(method = "glm", 
    method.args = list(family = "binomial"), 
...
## too much precip decreases survival
ggplot(sg_annual_scaled_merged, aes(ppt, surv, color = Br.x))+
  geom_point(pch="|")+
  geom_smooth(method = "glm", 
    method.args = list(family = "binomial"), 
    se = TRUE)+
...
ggplot(sg_annual_scaled_merged, aes(tmax, surv, color = Br.x))+
  geom_point(pch="|")+
  geom_smooth(method = "glm", 
    method.args = list(family = "binomial"), 
    se = TRUE)+
...
ggplot(sg_annual_scaled_merged, aes(tmin, surv, color = Br.x))+
  geom_point(pch="|")+
  geom_smooth(method = "glm", 
    method.args = list(family = "binomial"), 
    se = TRUE)+
...
lm1 <- glmer(surv~ Width.cm..x * Br.x + ppt +tmin + (1|NS), data = sg_annual_scaled_merged,
             family = binomial) 
lm2 <- glmer(surv~ Width.cm..x * Br.x + (1|NS), data = sg_annual_scaled_merged,
             family = binomial) 
lm3 <- glmer(surv~ (Width.cm..x + Br.x + tmin)^2 + (1|NS), data = sg_annual_scaled_merged,
             family = binomial) 
lm4 <- glmer(surv~ Width.cm..x + Br.x + ppt + (1|NS), data = sg_annual_scaled_merged,
             family = binomial) 
lm5 <- glmer(surv~ Width.cm..x + Br.x + ppt + tmin +
...
lm7 <- glmer(surv~ Width.cm..x + tmin + (1|NS), data = sg_annual_scaled_merged,
             family = binomial)  
lm8 <- glmer(surv~ Br.x + ppt + (1|NS), data = sg_annual_scaled_merged,
             family = binomial) 
...
p2 <- sg_annual_scaled_merged %>%
  ...
  dplyr::mutate(pptbin = cut(ppt, 
                              breaks = c(min(ppt)-0.1, mean(ppt), max(ppt)+0.1),
                              labels = FALSE)) %>%
  ggplot(  aes(tmin,surv, color = as.factor(pptbin)))+
     geom_point(pch = "|")+ ## mimic a rug plot
      geom_smooth(method = "glm",
                  method.args = list(family = "binomial"), se=TRUE)+
  facet_wrap(~NS)+
...
ggsave(paste(userpath, "Denver Botanic Gardens/Conservation - General/AllProjectsBySpecies/Sclerocactus-glaucus/Sclerocactus-glaucus_AnnualReports/",currentYr,"_Sclerocactus-glaucus_AnnualReport/",currentYr,"Fig4_SurvBrTminPPT.jpg", sep=""),
...
# write.csv(sg_sum, paste(userpath, "Denver Botanic Gardens/Conservation - General/AllProjectsBySpecies/Sclerocactus-glaucus/Sclerocactus-glaucus_AnnualReports/",currentYr,"_Sclerocactus-glaucus_AnnualReport/ScGl_density_correction.csv", sep=""))
