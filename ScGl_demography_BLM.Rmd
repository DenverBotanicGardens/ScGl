...
southsites <- c("Bridgeport","Escalante Canyon","Picnic Site","Powerline","Fram")
sg_summary08$NS[sg_summary08$SiteName %in% southsites] <- "S. glaucus"
...
sg.merged <- sg.new %>%
  ...
  left_join(sg.new, by = c("Site","Tag","Transect","X.coord.m.","Y.coord.m.")) %>%
...
ggsave(paste(userpath, "Denver Botanic Gardens/Conservation - General/AllProjectsBySpecies/Sclerocactus-glaucus/Sclerocactus-glaucus_AnnualReports/",currentYr,"_Sclerocactus-glaucus_AnnualReport/",currentYr,"densityXspecies.jpg", sep=""),

filter(sg_sum, !grepl("old", Site)) %>%
...
width = 230, height = 150, units = 'mm', dpi=300)
...
ggsave(paste(userpath, "Denver Botanic Gardens/Conservation - General/AllProjectsBySpecies/Sclerocactus-glaucus/Sclerocactus-glaucus_AnnualReports/",currentYr,"_Sclerocactus-glaucus_AnnualReport/",currentYr,"densityXspeciesPercentBaseline.jpg", sep=""),

    filter(sg_density, !grepl("old", Site)) %>%
...
width = 230, height = 150, units = 'mm', dpi=300)
  ...
  #   filter(sg_density, !grepl("old|Pond|Pyramid Rock", Site)) %>%
  #     ggplot(  aes(Year, Change.Density, color = Site))+
  #       geom_point(size=2)+
  #       geom_line()+
  # theme_bw()+
...
ggsave(paste(userpath, "Denver Botanic Gardens/Conservation - General/AllProjectsBySpecies/Sclerocactus-glaucus/Sclerocactus-glaucus_AnnualReports/",currentYr,"_Sclerocactus-glaucus_AnnualReport/",currentYr,"densityXspeciesPercentBaselineBysite.jpg", sep=""),      
filter(sg_density, !grepl("old", Site)) %>%
  ggplot(  aes(Year, Change.Density, color = NS))+
    ...
    geom_hline(yintercept = -0.2, col = "red"),
width = 330, height = 150, units = 'mm', dpi=300)
...
ggplot(sg_sum[sg_sum$Site %in% unique(grep(paste(oldnew,collapse="|"),
                   sg_sum$Site, value=TRUE)),], aes(Year,Density,colour=Site))+
  geom_point()+
  geom_line()+
  ylab("Total individuals")+
  theme_bw()
...
ggplot(scgl_season[grep("old",scgl_season$Site,invert=TRUE),], 
       aes(Year, PRep, group = Site, colour = Site)) +
  geom_point()+
  geom_line()+
...
ggplot(sg_season_scaled[!is.na(sg_season_scaled$Fl),], aes(Width.cm., as.numeric(Fl), colour = Br))+
  geom_point()+
  geom_smooth(method = "glm", 
    method.args = list(family = "binomial"), 
    se = FALSE)
...
lm1 <- glmer(Fl~ Width.cm. + Br + ppt_fall + ppt_spring + ppt_summer + ppt_winter +
             tmin_fall + tmin_spring + tmin_summer + tmin_winter + (1|NS:Site), data = sg_season_scaled,
             family = binomial) 
lm2 <- glmer(Fl~ Width.cm. + Br + (1|NS:Site), data = sg_season_scaled,
             family = binomial) 
lm3 <- glmer(Fl~ Width.cm. + Br + tmin_spring + tmin_winter + (1|NS:Site), data = sg_season_scaled,
             family = binomial) 
lm4 <- glmer(Fl~ Width.cm. + Br + ppt_fall + ppt_spring + ppt_summer + ppt_winter +
             (1|NS:Site), data = sg_season_scaled,
             family = binomial) 
lm5 <- glmer(Fl~ Width.cm. + Br + ppt_spring + ppt_summer + ppt_winter +
             (1|NS:Site), data = sg_season_scaled,
             family = binomial) 
lm6 <- glmer(Fl~ Width.cm. + Br + ppt_spring + ppt_summer + (1|NS:Site), data = sg_season_scaled,
             family = binomial)  
lm7 <- glmer(Fl~ Width.cm. + Br + ppt_spring + (1|NS:Site), data = sg_season_scaled,
             family = binomial)  
...
lm1 <- glmer(Fl~ Width.cm. * Br + ppt +tmin + tmax + (1|NS:Site), data = sg_annual_scaled,
             family = binomial) 
lm2 <- glmer(Fl~ Width.cm. * Br + (1|NS:Site), data = sg_annual_scaled,
             family = binomial) 
lm3 <- glmer(Fl~ (Width.cm. + Br + tmin)^2 + (1|NS:Site), data = sg_annual_scaled,
             family = binomial) 
lm4 <- glmer(Fl~ Width.cm. + Br + ppt + (1|NS:Site), data = sg_annual_scaled,
             family = binomial) 
lm5 <- glmer(Fl~ Width.cm. + Br + ppt + tmin + tmax +
             (1|NS:Site), data = sg_annual_scaled,
             family = binomial) 
lm6 <- glmer(Fl~ Width.cm. + ppt + (1|NS:Site), data = sg_annual_scaled,
             family = binomial)  
lm7 <- glmer(Fl~ Width.cm. + tmin + (1|NS:Site), data = sg_annual_scaled,
             family = binomial)  
lm8 <- glmer(Fl~ (Width.cm. + Br + ppt)^2 + (1|NS:Site), data = sg_annual_scaled,
             family = binomial)
...
# more percent reproductive with avg width
summary(lm3)
sjPlot::plot_model(lm3, type = "est", show.data = T)
...
sg_annual_scaled %>%
  filter(!is.na(Fl)) %>%
  dplyr::mutate(SizeClass = ifelse(Width.cm. < 3.04 & NS == "S. dawsonii", "veg", "rep"),
                SizeClass = ifelse(Width.cm. < 2.28 & NS == "S. glaucus", "veg", "rep")) %>%
ggplot(aes(tmin, Fl, colour = Br))+
  geom_point() +
  geom_smooth(method = "glm", 
    method.args = list(family = "binomial"), 
    se = FALSE)+
  facet_wrap(SizeClass~NS, scales = "free")

effectsize::standardize(lm1) %>% summary()
sjPlot::tab_model(lm3)
p1 <- sjPlot::plot_model(lm3, type = "int", mdrt.values = "meansd", colors = c("blue","orange","brickred"))
p1[[1]]$labels$title <- ""
p1[[1]]$labels$colour <- "Browsing"
p1[[1]]$labels$x <- "Size (width cm)"
p1[[1]]$labels$y <- "Probability of reproduction"
p1[[2]]
