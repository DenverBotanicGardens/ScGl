...
## Gonna say that all the Powerline (old) are just repeats of the Powerline and can remove
sg <- sg %>%
  filter(Site != "Powerline (old)")
...
## Number of ScGl counted in currentYr
...
table(sg$Site,sg$Year) ## Three individual records that aren't assigned sites!
...
## Don't remove, assign correctly! In MySQL phpMyAdmin
#   ScGl_data_id Site Year Tag Transect X.coord.m. Y.coord.m. Degrees Direction Height.cm. Width.cm. Fl Br Minis PhotoNumber       Comments
# 1        30747      2024  NA                  NA         NA      NA        NA        4.0       6.9  y  n    NA           5 tag is 70cm S.
# 2        30748      2024  NA                  NA         NA      NA        NA        1.8       1.3  n  n     1             tag is 68cm S.
# 3        32153      2025  NA                  NA         NA      NA        NA        1.5       1.9  n  n    NA                           
#### 32153 could be tag 77.05 in tran ID 3, transect 2, pyramid rock?? that was new, existing tag was 1183 in Pyramid Rock
## 1 tag id: 4158 - tag ID missing
## 2 tag id: 4159 
## 3 tag id 3984 tag number	1183 match (ID 2, transect 1 Pyramid rock) 
## OR tag id 165969 , the largest number that's in there is 4387
## Has tag ids 3902, 3667, and the 165969
# sg <- sg %>%
#   mutate(Tag = case_when(ScGl_data_id == 30747 ~ 414.06,
#                          ScGl_data_id == 30748 ~ 414.07,
#                          ))
...
sg %>%
  ...
  mutate(NS = case_when(Site %in% c("Atwell Gulch_108","Atwell Gulch_165", "Pond",
                                    "Oil Pad","Pyramid Rock", "T-Junction") ~ "SCDA",
                        TRUE ~ "SCGL")) %>%
ggplot(   aes(Width.cm., log(FlNum), color = NS))+
  stat_smooth(method = "glm") +
  coord_cartesian(ylim = c(0,5)) +
  geom_point() +
  facet_grid(~Year, scales = "free") +
  theme_bw() +
  ylab("Flowers per Individual (log transformed)")+
  xlab("Width (cm)") +
  scale_color_manual("Species", values = c("olivedrab","grey30"))
...
```{r}
# Add a level to the Site factor field, adding Atwell Gulch and Atwell Gulch old but keeping _108 and _165 new and old
sg$Site <- factor(sg$Site, 
                  levels = c("Atwell Gulch", "Atwell Gulch (old)",levels(sg$Site)))

# Rename all Atwell Gulch in the Site column
sg[grep("Atwell", sg$Site), "Site"] <- "Atwell Gulch"

## Are the X and Y swapped? No, the X should be *0.01? 
sg[sg$X.coord.m. > 1.1 & !is.na(sg$X.coord.m.) & sg$Site == "Atwell Gulch",]
sg[sg$X.coord.m. > 1.1 & !is.na(sg$X.coord.m.) ,] ## Some Fram that are crazy, then oil pad old , so only do Atwell Gulch
sg <- sg %>%
  mutate(X.coord.m. = case_when((X.coord.m. > 1.1 & !is.na(X.coord.m.) & Site == "Atwell Gulch") ~ X.coord.m.* 0.01,
                                TRUE ~ X.coord.m.))
...
# Assign genetic regions; now new species, still have not seen the publication 2022; Mcglaughlin & Naibauer 2024 NOVON  
## Here everything is Atwell Gulch, need to add a copy of (old) with Y <= 30 later
sg <- sg %>%
  mutate(NS = case_when(grepl("Bridgeport", Site) ~ "S. glaucus",
                        grepl("Escalante", Site) ~ "S. glaucus",
                        grepl("Picnic", Site) ~ "S. glaucus",
                        Site %in% c("Powerline","Fram") ~ "S. glaucus",
                        TRUE ~ "S. dawsoniae")) %>%
...
## Add Site Same for linking climate data
sg <- sg %>%
  mutate(SiteSame = case_when(grepl("Atwell", Site) ~ "Atwell Gulch",
                              ...
                              grepl("Powerline", Site) ~ "Powerline",
                              grepl("Pyramid", Site) ~ "Pyramid Rock",
                              grepl("Picnic", Site) ~ "Picnic Site",
                              grepl("Escalante", Site) ~ "Escalante Canyon",
                              TRUE ~ Site)) %>%
...
table(sg_summary08$Site, sg_summary08$NS)
...
ggplot(atwell.old, aes(X.coord.m., Y.coord.m., color = as.factor(Year)))+
  geom_point(position = position_dodge(width = 0.04), pch="|",size = 2)+
  facet_wrap(~ Transect, nrow = 2, scales = "free_x")+
  theme_bw()

# There is no transect 1! There is no 2! In MySQL Transect ID 1 and 2 are bridgeport
# Transect 1 --> 4.4
## 138.02 should be transect 4.4 (not 1) 
## 453.00 also 4.4 (ID 25)
# Transect 2 --> 9.09
## 34 is 9.09 
## 137.01 and 137.02 9.09
## ...
table(atwell.old$Transect)
sg %>%
  filter(Site == "Atwell Gulch" & Transect %in% c("1","2")) %>%
  group_by(Tag, Transect) %>%
  summarize(n = n()) %>%
  arrange(Transect, Tag) %>%
  print(n = 25)

sg <- sg %>%
  mutate(Transect = case_when((Transect == "1" & Site == "Atwell Gulch") ~ "4.4",
                              (Transect == "2" & Site == "Atwell Gulch") ~ "9.09",
                              TRUE ~ Transect)) 
...
sg_summary08 <- sg_summary08 %>%
  dplyr::mutate(Height95HDIupr = NA, Width95HDIupr = NA) %>%
...
What should the y-intercept line be?
...
plot2 <- filter(sg_density, !grepl("old", Site)) %>%
      ggplot(  aes(Year, Change.Density, color = Site))+
        ...
        geom_hline(yintercept = -0.2, linetype = "longdash")  # A 20% decline in baseline
...
sg_density %>%
  ...
  mutate(PercofBase = MeanChange - 1)
...
Figure 7 Density baseline changes
```{r}
...
## The four that have old and new
...
ggplot(sg[which(sg$Width.cm. > 0 & !duplicated(sg[,c("Tag","SiteSame")])),], aes(NS,Width.cm., color = as.factor(Fl)))+
  geom_boxplot()+
  ylim(c(0,17))+
  geom_point(position = position_jitterdodge(jitter.height = 0))+
  scale_x_discrete(labels = label_wrap(5)) +
...
sg_sum %>%
  ...
  mutate(PRep = Rep/X) %>%
  group_by(NS) %>%
  summarise(Mean = mean(PRep), SD = sd(PRep))
...
scgl.climate.monthly %>%
  group_by(Variable, Site, Year, season) %>%
  summarise(n = n()) %>%
  filter(n > 3)

scgl.climate.monthly %>%
  filter(Variable == "tmax" & Site == "Atwell Gulch" & Year == 2024) %>%
  arrange(Month, season)
## Some repeated months with slightly different values, some the same value. Maybe provisional and updated? 


## More tmax and tmin than ppt. 
scgl.c <- scgl.climate.monthly %>%
  distinct(Variable, Site, NS, Year, Month, Prev12, season, .keep_all = TRUE) %>%
  pivot_wider(names_from = Variable, values_from = data) 


## Remove any possible duplicate values so the sum doesn't get out of wack
...
## Getting >8700 missing values, the join isn't working 
## SiteSame gets it down to only >1000 NAs for climate  
## GO BACK, must have assigned SCGL and SCDA wrong somewhere!! 
sg_season <- sg %>%
  left_join(scgl.season.wide, join_by("Year" == "Prev12", "SiteSame" == "Site", "NS" == "NS"))
sg_annual <- sg %>%
  left_join(scgl.annual.wide, join_by("Year" == "Prev12", "SiteSame" == "Site", "NS" == "NS"))

## Only missing values are previous to 1986
scgl.season.wide %>%
  filter(is.na(ppt_fall)) %>%
  print(n = 100)

scgl.annual.wide %>%
  filter(is.na(Value_ppt))
...
## Got rid of all no climate data, back to good!!
sg_season %>%
  filter(is.na(tmin_winter))  ###AAAAh, now have > 8700 rows with no climate data!!! (in 2025)
### Minimum width of reproductive by species  
sg %>%
  group_by(NS, SiteSame, Year) %>%
  filter(Fl == 1) %>%
  filter(Width.cm. > 1) %>%
  summarise(minRep = min(Width.cm.),
            median = median(Width.cm.)) %>%
  group_by(NS) %>%
  summarise(meanMin = mean(minRep),
            meanMedian = mean(median))
  
## Clearly entered wrong, just change them to same as Height for analyses. 
sg <- sg %>%
  mutate(Width.cm. = case_when((Fl == 1 & Width.cm. < 1 & Height.cm. > 2) ~ Height.cm.,
                               TRUE ~ Width.cm.))
sg %>%
  filter(ScGl_data_id %in% c(25837, 29838,7463 ))
```
...
brflowered <- sg_season %>%
  filter(!is.na(Fl)) %>%
  filter(Width.cm. > 1) %>% ## Some with wrong measurements for width
...
sg_annual %>%
  filter(is.na(scalevalue_tmax))
```  

Survival - need merge year to year like what I made for Dan Doak but add a survived or not
```{r}

## Here, the climate are the 12 months previous to Year.x, need it to be to Year.y
...
sg_annual_scaled_merged <- sg.new %>%
  # group_by(Site, SiteSame, NS, Transect, Tag, Year) %>%
  left_join(sg_annual, by = c("Site","SiteSame","NS","Tag","Transect")) %>%
  dplyr::select(NS,Year.x:Transect, Width.cm..x:Minis.x, Year.y, Width.cm..y, Value_ppt:scalevalue_tmin) %>%
  filter(Year.x == Year.y-1) %>%
  filter(Width.cm..x != 0) %>%
  mutate(surv = ifelse(Width.cm..y ==0, 0,1))%>%
  mutate(Br.x = as.factor(Br.x))
...
So few browsed, took that out of the analyses. 
Is the weather for the months before Year.y as it should be? - No, I think it's for two years before! 
```{r}
...
ggsave(paste(userpath, "Denver Botanic Gardens/Conservation - General/AllProjectsBySpecies/Sclerocactus-glaucus/Sclerocactus-glaucus_AnnualReports/",currentYr,"_Sclerocactus-glaucus_AnnualReport/",currentYr,"Fig_Survival.jpg", sep=""),p1a_woBr, width = 200, height = 110, units = 'mm', dpi=300)
...
sg_annual_scaled_merged %>%
  ...
  dplyr::mutate(scalevalue_pptbin = cut(scalevalue_ppt, 
                              breaks = c(min(scalevalue_ppt)-0.1, mean(scalevalue_ppt), max(scalevalue_ppt)+0.1),
                              labels = FALSE)) %>%
  ggplot(  aes(Width.cm..x,surv, color = NS, linetype = as.factor(scalevalue_pptbin)))+
     geom_point(pch = "|")+ ## mimic a rug plot
      geom_smooth(method = "glm",
                  method.args = list(family = "binomial"), se=TRUE)+
  facet_grid( ~ tempbin, 
    ...
    theme_dark()+
    scale_color_manual("Species", values = c("white","deeppink4"),
                         # labels = c("Unbrowsed","Browsed")
                       )+
  scale_linetype_manual("Annual PPT", labels = c("<= mean ppt", "> mean ppt"), values = c("solid","dotted")) +
    ylab("Survival")+
    xlab("Size (width cm)")+
  ggtitle("a)")
...
ggplot(  aes(scalevalue_ppt,surv, color = as.factor(tempbin)))+
  ...
  facet_wrap(~NS)+
  theme_light()+
  scale_color_manual("Temperature", values = c("blue","red"),  # Minimum\nTemperature
                     labels = c("1" = "<= mean\nmin temp", "2" = "> mean\nmin temp"))+
  ylab("Survival")+
  xlab("Precipitation")+
  ggtitle("b)")
...
p2 <- sg_annual_scaled_merged %>%
 ...
 dplyr::mutate(pptbin = cut(scalevalue_ppt, 
  ...
  facet_wrap(~NS)+
    theme_light()+
    scale_color_manual("Precipitation", values = c("brown","darkgreen"),
                         labels = c("1" = "<= mean\nppt", "2" = "> mean\nppt"))+
    ylab("Survival")+
...
ggsave(paste(userpath, "Denver Botanic Gardens/Conservation - General/AllProjectsBySpecies/Sclerocactus-glaucus/Sclerocactus-glaucus_AnnualReports/",currentYr,"_Sclerocactus-glaucus_AnnualReport/",currentYr,"Fig4_SurvBrscalevalue_tminPPT.jpg", sep=""),
       p1a_woBr  + (p1 / p2), width = 300, height = 160, units = 'mm', dpi=300) 
...
sg_annual_scaled_merged %>%
  ...
  summarise(n = n()) %>%
  ungroup() %>%
  mutate(PercBr = n[Br.x == 1]/n[Br.x == 0])
...
sg_annual_FlAIC <- sg_annual %>%
  mutate(Br = as.factor(Br)) %>%
  filter(Width.cm. > 1) %>%
  filter(!grepl("old", Site))

lm1 <- glmer(Fl~ Width.cm. + scalevalue_ppt +scalevalue_tmin + (1|NS), data = sg_annual_FlAIC,
             family = binomial) 

lm2 <- glmer(Fl~ Width.cm. + (1|NS), data = sg_annual_FlAIC,
             family = binomial)

lm3 <- glmer(Fl~ (Width.cm. + scalevalue_tmin)^2 + (1|NS), data = sg_annual_FlAIC,
             family = binomial)
lm8 <- glmer(Fl~ (Width.cm. + scalevalue_ppt)^2 + scalevalue_tmin + (1|NS), data = sg_annual_FlAIC,
             family = binomial)

lm4 <- glmer(Fl~ Width.cm. + scalevalue_ppt + (1|NS), data = sg_annual_FlAIC,
             family = binomial)

lm7 <- glmer(Fl~ Width.cm. + scalevalue_tmin + (1|NS), data = sg_annual_FlAIC,
             family = binomial)  

# lm9 <- glmer(Fl ~ Width.cm. + (1|NS), data = sg_annual_FlAIC,
#              family = binomial)

lm10 <- glmer(Fl ~ 1 + (1|NS), data = sg_annual_FlAIC,
             family = binomial)

# lm6 <- glmer(Fl~ Width.cm. + scalevalue_ppt + (1|NS), data = sg_annual_FlAIC,
#              family = binomial)  

lm.list <- list(lm1, lm2, lm3, # lm4,
                lm8, lm7, lm10) # lm6 lm9, 
lm.names <- as.character(unlist(lapply(lm.list,formula)))
(lm.results <- aictab(lm.list, modnames=lm.names))
#evidence ratio 
for(i in 2:length(lm.list)){
...
Fig5b <- sg_annual %>%
  ...
  mutate(SizeClass = cut(Width.cm., breaks = c(0.5, 3, 6, max(Width.cm.)) )) %>%
  # filter(SizeClass == 2) %>%
ggplot(aes(scalevalue_tmin, Fl, colour = NS))+
  geom_point(pch="|") +
  geom_smooth(method = "glm", 
  ...
  ggtitle("b)")
# labeller = labeller(SizeClass = as_labeller(c("(0,5.3]" = "< 3 cm width", 
#                                                            "3 - 6 cm width",
#                                                             "> 6 cm width"))))
...
Fig5a <- sg_annual %>%
  ...
  filter(!is.na(Width.cm.)) %>%
  filter(Width.cm. > 1) %>% # dead
  # group_by(NS) %>%
  dplyr::mutate(tempbin = cut(scalevalue_tmin, 
                              # breaks = c(min(scalevalue_tmin)-0.01, -0.5, 0, 0.5, max(scalevalue_tmin)+0.01),
                              breaks = c(min(scalevalue_tmin)-0.01, median(scalevalue_tmin),
                                         max(scalevalue_tmin)+0.01),
                              labels = FALSE)) %>%
    ggplot(  aes(Width.cm., Fl, color = NS))+
      geom_point(pch = "|")+ ## mimic a rug plot
      geom_smooth(method = "glm",
                  method.args = list(family = "binomial"), se=TRUE)+
      facet_grid(~tempbin, labeller = labeller(tempbin = as_labeller(c("1" = "<= mean min temp",
                                "2" = "> mean min temp"))
                      # NS = as_labeller(c("S. glaucus" = expression(italic(S.~glaucus)),
                      #                    "S. dawsoniae" = expression(italic(S.~dawsoniae))))
                      ))+
...
ggsave(paste(userpath, "Denver Botanic Gardens/Conservation - General/AllProjectsBySpecies/Sclerocactus-glaucus/Sclerocactus-glaucus_AnnualReports/",currentYr,"_Sclerocactus-glaucus_AnnualReport/",currentYr,"Fig5_ProbofReproductive.jpg", sep=""),
       Fig5a + Fig5b, width = 300, height = 125, units = 'mm', dpi=300)
...
Fig3a <- sg_sum %>%
  filter(!(Site %in% c("Bridgeport (old)", "Escalante Canyon (old)",
                       "Picnic Site (old)", "Road T-West (old)", "Pyramid Rock (old)"))) %>%
  mutate(Site = as.factor(Site)) %>%
ggplot(   aes(Year, X/area, colour = Site)) +
  geom_line() +
  geom_point() +
  ggtitle("a)")+
...
Fig3b <- sg_sum %>%
  filter(!(Site %in% c("Bridgeport (old)", "Escalante Canyon (old)",
                       "Picnic Site (old)", "Road T-West (old)", "Pyramid Rock (old)"))) %>%
  mutate(Site = as.factor(Site)) %>%
ggplot(   aes(Year,avgVol,colour=Site))+
  geom_line()+
  geom_point()+
  ggtitle("b)")+
...
Fig3c <- sgminis %>%
  filter(!(Site %in% c("Bridgeport (old)", "Escalante Canyon (old)",
                       "Picnic Site (old)", "Road T-West (old)", "Pyramid Rock (old)"))) %>%
  mutate(Site = as.factor(Site)) %>%
ggplot(     aes(Year, TotMinis.x,colour= Site))+
  geom_line()+
  geom_point()+
  ggtitle("c)")+
  facet_wrap(~NS + SiteSame, ncol =5)+ # , scales = "free_y"
...
Fig3a/Fig3b/Fig3c,
width=275, height=325, units="mm", dpi=300)
...
ggheight <- sg.xy %>%
  filter(!grepl("old", Site)) %>%
  filter(Height.cm..x < 30) %>%
  filter(Height.cm..y < 30) %>%
...
SiteSp <- sg %>%
  ...
  mutate(SiteSame = case_when(grepl("Atwell", SiteSame) ~ "AtwellGulch",
                              grepl("Escalante", SiteSame) ~ "EscalanteCanyon",
                              grepl("Oil", SiteSame) ~ "OilPad",
                              grepl("Picnic", SiteSame) ~ "PicnicSite",
                              grepl("Pyramid", SiteSame) ~ "PyramidRock",
                              grepl("Junction", SiteSame) ~ "TJunction",
                              TRUE ~ SiteSame))


scglddllambda$Phi <- scglddllambda$Phi %>%
  left_join(SiteSp, by = c("Site"="SiteSame"))

scglddllambda$p <- scglddllambda$p %>%
  left_join(SiteSp, by = c("Site"="SiteSame"))
  
scglddllambda$Lambda <- scglddllambda$Lambda %>%
  left_join(SiteSp, by = c("Site"="SiteSame"))
...
Detection less than perfect at all sites?  
```{r}
scgl.detection <- function(){
  Lambda.site = list(formula = ~ 1)
  Phi.site = list(formula = ~ 1)
  p.dot = list(formula = ~ 1,
               fixed = list(index = p.indices, value = p.values))
  p.site = list(formula = ~ Site,
               fixed = list(index = p.indices, value = p.values))
  p.species = list(formula = ~ NS,
               fixed = list(index = p.indices, value = p.values))
  
  scgl.model.list <- create.model.list("Pradlambda")
  scgl.results <- mark.wrapper(scgl.model.list,
                               data = scglproclambda,
                               ddl = scglddllambda)
...
scgl.model <- scgl.detection()
save(scgl.model, file = paste(userpath, "Denver Botanic Gardens/Conservation - General/AllProjectsBySpecies/Sclerocactus-glaucus/Sclerocactus-glaucus_AnnualReports/",currentYr,"_Sclerocactus-glaucus_AnnualReport/",currentYr,"PradlambdaDetection.Rdata", sep=""))

p.table <- get.real(scgl.model$Phi.site.p.site.Lambda.site, "p", se = TRUE)
p.table %>%
  distinct(estimate, se, lcl, ucl, group) %>%
  filter(estimate > 0) %>%
  arrange(estimate)
...
### "In detection corrected estimates of survival and recruitment, where detection, recruitment, and survival differed by site ""
get.real(scgl.constantLambda, "Phi", se = TRUE) %>%
  filter(fixed != "Fixed") %>%
  left_join(SiteSpecies) %>%
  group_by(NS) %>%
  summarise(lambda = mean(estimate),
...
ggsave(paste(userpath, "Denver Botanic Gardens/Conservation - General/AllProjectsBySpecies/Sclerocactus-glaucus/Sclerocactus-glaucus_AnnualReports/",currentYr,"_Sclerocactus-glaucus_AnnualReport/",currentYr,"Pradel_BLM-FWS.jpg", sep=""),
lambda.table %>%
  filter(fixed != "Fixed") %>%
  left_join(SiteSpecies) %>%
  group_by(Site, NS, SiteLabel) %>%
...
run.lambda <- function(){
  ...
  Lambda.species = list(formula = ~ NS)
...
### ????  Phi.Plot.p.Site.Lambda.Plottime doesn't make sense
lambda.table <- get.real(scgl.lambda$Phi.Plot.p.Site.Lambda.Plot, "Lambda", se = TRUE)
lambda.table %>%
  filter(fixed != "Fixed") %>%
  left_join(SiteSpecies) %>%
  distinct(estimate, lcl, ucl, SiteLabel, NS) %>%
...
## In detection corrected estimates of survival and recruitment, where detection, recruitment, and survival differed by site...
get.real(scgl.lambda$Phi.species.p.Site.Lambda.Plot, "Phi", se = TRUE) %>%
  filter(fixed != "Fixed") %>%
  left_join(SiteSpecies) %>%
  distinct(estimate, lcl, ucl, NS)
