...
lm3 <- glm(Prepro~NS*avgW*data.ppt,
            family = "binomial", sg_sum, weights = X)
lm4 <- glm(Prepro~NS*avgW*data.ppt*data.tmax,
            family = "binomial", sg_sum, weights = X)
...
sjPlot::tab_model(lm11, transform = NULL,   # estimates on linear scale
                  pred.labels = c("Intercept", "size","ppt","max temp",
                                  "size:ppt","size:Max Temp","ppt:max temp",
                                  "size:ppt:max temp"),
                  string.ci = "CI (95%)",
                  string.p = "p-value",
                  show.reflvl = TRUE)  # show reference level for categorical (none here)

# sg_sum$annual.tminbin <- cut_width(sg_sum$data.tmin, 5) 
# 
...
#AIC
## find best fit model for each part - reproduction, survival of juveniles, survival of adults
```{r}
