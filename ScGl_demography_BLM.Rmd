...
scgl.constant <- mark(data = scglproc, ddl = scglddl, model = "Pradrec",
                      model.parameters = list(p = list(formula = ~ Species, 
                                                       fixed = list(index = p.indices, value = p.values)),
                                              Phi = list(formula = ~ Site*Species*time),
                                              f = list(formula = ~ time*Site*Species)),
                      begin.time = 2008,
                      groups = "Site")

summary(scgl.constant)

nrow(scgl.constant$results$real) ## 303
scgl.constant$results$real %>%
  filter(fixed != "Fixed") %>%
  summarise(n = n())## 302
nrow(scgl.constant$results$derived$`log(Lambda) Population Change`) ## 150



Phi.table <- get.real(scgl.constant, "Phi", se = TRUE)
nrow(Phi.table) ## 150
Phi.table %>%
  filter(fixed != "Fixed") %>%
  group_by(Site)
...
Fig2b <- scgl.constant$results$derived$`log(Lambda) Population Change` %>%
  bind_cols(Phi.table[,c("group","age","time","Time","Site")]) %>%
  left_join(SiteSpecies) %>%
  group_by(group, NS) %>%
  dplyr::summarise(hdilwl = HDInterval::hdi(estimate)[1],
                   hdiupl = HDInterval::hdi(estimate)[2],
                   median = median(estimate)) %>%
  ggplot(  aes(group, median, color = NS))+
    geom_point()+
