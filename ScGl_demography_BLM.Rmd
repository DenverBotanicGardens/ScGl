...
# table(sg$Year[ grep("Atwell Gulch",sg$Site)]) 
...
ggplot(sg[sg$Year > 2018,], aes(Site, FlNum, color = Site, group = Site))+
...
# Missing values in 2015, need imputing
...
ggplot(sg_sum, aes(Year,X,colour=Site))+
  ...
  # geom_dag_label_repel(aes(label = Site), nudge_x = 1, na.rm = TRUE, label.padding = 0 )
  annotate("text", x = aggregate(Year ~ Site, sg_sum, max)$Year + 1.5,
           y = aggregate(X ~ Site, sg_sum, FUN = tail, n = 1)$X,
           label = paste(unique(sg_sum$Site[sg_sum$X %in% aggregate(X ~ Site,sg_sum, 
                                              FUN = tail, n = 1)$X &
