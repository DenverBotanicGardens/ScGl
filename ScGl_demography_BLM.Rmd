...
```{r}
rm(sg)
#sg <- read.csv(path.expand("P:/hackathon/ScGl/2015_ScGl_rawdata_witherrors.csv"))
#sg <- read.csv(path.expand("P:/hackathon/ScGl/2015_ScGl_rawdata_102915.csv"))
#sg <- read.csv(path.expand("Q:/Research/All_Projects_by_Species/Sclerocactus SPECIES/Sclerocactus_glaucus/Excel/2015_ScGl/RawData_scgl_2015_102915.csv"))
# 2015
...
sg <- read.csv(path.expand("Q:/Research/All_Projects_by_Species/Sclerocactus SPECIES/Sclerocactus_glaucus/R_code_ScGl/R_tables/RawData_scgl_2016.csv"))
names(sg)
...
2008:      
```{r}
table(scgl08_2$SiteName)  #summarized number of plants per plot per site
...
levels(sg$Site) <- c("Atwell Gulch", "Atwell Gulch (old)",levels(sg$Site))
...
#Exclude Atwell and Road T-East
sgNotatwell <- sg[grep("Atwell", sg$Site, invert=TRUE),]
table(sgNotatwell$Site,sgNotatwell$Year)
noEast <- sgNotatwell[grep("Road T-East", sgNotatwell$Site, invert=TRUE),]
table(noEast$Site,noEast$Year)

noEast <- noEast[-intersect(grep("old", noEast$Site), which(noEast$Year>2015)),]
noEast <- noEast[-intersect(grep("Pond", noEast$Site), which(noEast$Year == 2015)),]

```

#Data checks
```{r}
ggplot(noEast[noEast$Site == "Picnic Site",], aes(Transect,Height.cm.))+
  geom_point()
#WHAAAaaaaa Picnic Site got Pond's transects?
str(noEast)
...
```{r}
#bind new and old Atwell to sg
sg_all <- rbind(noEast,atwell,atwell_old)
#get rid of Atwell Gulch_108 and Atwell Gulch_165 now unused factor levels
sg_all$Site <- droplevels(sg_all$Site)

table(sg_all$Site,sg_all$Year)
str(sg_all)

# All sites 
sg_summary <- ddply(sg_all,  
                    .(Site,Year), summarise,
...
get_prism_monthlys(type="ppt", mon = 1:12, keepZip = FALSE, years = 2000:2015)

# Maximum temperature (average)
get_prism_monthlys(type="tmax", mon = 1:12, keepZip = FALSE, years = 2000:2015)

# Minimum temperature (average)
get_prism_monthlys(type="tmin", mon = 1:12, keepZip = FALSE, years = 2000:2015)
```
...
```{r}
latlong <- data.frame(rbind(c(39.1969, -108.1298, "Atwell"),    
c(38.8491, -108.3717, "Bridgeport"),   
c(38.6593, -108.3466, "Escalante"),     
c(39.0612, -108.3799, "Fram"),    
c(39.3446, -108.3102, "Oil Pad"),    
c(38.6701, -108.3286, "Picinic"),    
...
Climate data from PRISM <http://prism.oregonstate.edu/explorer/>

```{r, eval=FALSE}
at.cli<-read.csv(path.expand("Q:/Research/All_Projects_by_Species/Sclerocactus SPECIES/Sclerocactus_glaucus/R_code_ScGl/R_tables/Atwell_PRISM_2015.csv"),skip=6)
br.cli<-read.csv(path.expand("Q:/Research/All_Projects_by_Species/Sclerocactus SPECIES/Sclerocactus_glaucus/R_code_ScGl/R_tables/Bridgeport_PRISM_2015.csv"),skip=6)
es.cli<-read.csv(path.expand("Q:/Research/All_Projects_by_Species/Sclerocactus SPECIES/Sclerocactus_glaucus/R_code_ScGl/R_tables/Escalante_PRISM_2015.csv"),skip=6)
fr.cli<-read.csv(path.expand("Q:/Research/All_Projects_by_Species/Sclerocactus SPECIES/Sclerocactus_glaucus/R_code_ScGl/R_tables/Fram_PRISM_2015.csv"),skip=6)
op.cli<-read.csv(path.expand("Q:/Research/All_Projects_by_Species/Sclerocactus SPECIES/Sclerocactus_glaucus/R_code_ScGl/R_tables/OilPad_PRISM_2015.csv"),skip=6)
pic.cli<-read.csv(path.expand("Q:/Research/All_Projects_by_Species/Sclerocactus SPECIES/Sclerocactus_glaucus/R_code_ScGl/R_tables/Picnic_PRISM_2015.csv"),skip=6)
power.cli<-read.csv(path.expand("Q:/Research/All_Projects_by_Species/Sclerocactus SPECIES/Sclerocactus_glaucus/R_code_ScGl/R_tables/Powerline_PRISM_2015.csv"),skip=6)
pond.cli<-read.csv(path.expand("Q:/Research/All_Projects_by_Species/Sclerocactus SPECIES/Sclerocactus_glaucus/R_code_ScGl/R_tables/Pond_PRISM_2015.csv"),skip=6)
pr.cli<-read.csv(path.expand("Q:/Research/All_Projects_by_Species/Sclerocactus SPECIES/Sclerocactus_glaucus/R_code_ScGl/R_tables/PyramidRock_PRISM_2015.csv"),skip=6)
tj.cli<-read.csv(path.expand("Q:/Research/All_Projects_by_Species/Sclerocactus SPECIES/Sclerocactus_glaucus/R_code_ScGl/R_tables/TJunction_PRISM_2015.csv"),skip=6)

at.cli<-data.frame(Site="Atwell Gulch",at.cli)
br.cli<-data.frame(Site="Bridgeport",br.cli)
es.cli<-data.frame(Site="Escalante Canyon",es.cli)
fr.cli<-data.frame(Site="Fram",fr.cli)
op.cli<-data.frame(Site="Oil Pad",op.cli)
pic.cli<-data.frame(Site="Picnic Site",pic.cli)
power.cli<-data.frame(Site="Powerline",power.cli)
pond.cli<-data.frame(Site="Pond",pond.cli)
pr.cli<-data.frame(Site="Pyramid Rock",pr.cli)
tj.cli<-data.frame(Site="T-Junction",tj.cli)

scgl.climate <- rbind(at.cli,br.cli,es.cli,fr.cli,op.cli,pic.cli,power.cli,pond.cli,pr.cli,tj.cli)
scgl.climate$Year <- substr(scgl.climate$Date,1,4)
scgl.climate$Month <- substr(scgl.climate$Date,6,7)
...
```{r}
scgl.c$Month <- as.numeric(scgl.c$Month)
scgl.c$Year <- as.numeric(scgl.c$Year)

#Months after data collection to be assigned to next year
#Add a column to assign the year from May to the previous 12 months: Jun:May
scgl.c$Prev12 <-scgl.c$Year
#Months June through December go to the next year
scgl.c[scgl.c$Month > 5,"Prev12"] <- scgl.c[scgl.c$Month > 5,"Year"]+1
 
scgl.c[scgl.c$Prev12 >2014,]
```
...
scgl.c.summary <- aggregate(scgl.c[,2:4], scgl.c[,c("Site","Prev12","season")],
                            FUN = function(x) mean(x, na.rm = TRUE))

season <- reshape(scgl.c.summary[scgl.c.summary$Prev12<2016,],
                  idvar = c("Site","Prev12"),
                  timevar = "season",
                  direction = "wide")
...
summary statistics
```{r}
scgl_sumSE <- lapply(names(scgl.c[,c("Precip","tmin", "tmax")]), 
                     function(x){
...
Take out missing data, Years 1986 through 2015 as previous 12 years to the May survey
...
str(cc.season)
nrow(cc.season)
unique(sort(cc.season$Prev12)) #1986:2015
...
Ditch anything 50% or more correlated 
  keep  Precip.fall   Remove: tmax.fall(-0.64)
        tmin.fall             tmin.spring(0.56)
                              tmin.summer(0.56)
        Precip.spring         tmax.spring(-0.53)
        Precip.summer         tmax.summer(-0.58)
        Precip.winter         
        tmin.winter           tmax.winter(0.8)
...
ggplot(sg.pc, aes(Comp.1,Comp.2, colour=Site))+
  geom_point() +
  stat_ellipse() +
  theme_bw() 
...
levels(sg.pc$Site)[levels(sg.pc$Site) == 'T-Junction'] <- 'TJunction'
...
#### Figure 3: The Principal Components Analysis (PCA) of climatic variables with loadings of summer and spring temperatures and Summer Palmer Z Index grouped by a) genetic region and b) site. c) Site distribution along PC1 is shown with Tukey Honest Significant Differences. 
```{r}
ggplot(sg.pc, aes(Comp.1,Comp.2, colour=NS))+
  geom_point() +
  stat_ellipse() +
...
```{r}
recent <- season[season$Prev12 > 2007 & season$Prev12 < 2016,]
...
#### 2008-2015
Ditch anything 50% or more correlated 
Keep Precip.fall and  Remove: tmax.fall(-0.62) 
                              Precip.summer(0.53)
     tmin.fall                tmin.spring(0.56)
                              tmin.summer(0.87)
     Precip.spring            tmax.spring(-0.55)
     tmin.spring              Precip.winter(-0.70)
     tmin.summer              
     tmax.summer
     tmax.winter              Precip.winter(-0.66)
...
forpca <- recent[,c(grep(paste(tomatch,collapse="|"),
...
sg.pc <- data.frame(scgl.pc,recent)
# Assign genetic regions
sg.pc$NS <- "North"
southsites <- c("Bridgeport","Escalante","Picnic","Powerline","Fram")
sg.pc$NS[sg.pc$Site %in% unique(grep(paste(southsites,collapse="|"),
...
Figure 3 f) Site distribution along PC1 is shown with Tukey Honest Significant Differences. 

```{r}
#doesn't like "-" in the name
levels(sg.pc$Site)[levels(sg.pc$Site) == 'T-Junction'] <- 'TJunction'
sg.thsd <- TukeyHSD(aov(Comp.2~Site, data=sg.pc))
lasign <- multcompLetters(extract_p(sg.thsd$Site))


SiteLetters <- data.frame(Letter = lasign$Letters[],
                          Site = rownames(data.frame(lasign$Letters)))
Siteletters <- SiteLetters[order(SiteLetters$Site),]

ggplot(sg.pc, aes(Site, Comp.2))+
  geom_boxplot()+
  annotate("text",x=1:10, y = 3,
           label = SiteLetters$Letter[c(10,1:9)])
```
...
```{r Figure 2 pond only}
ggplot(sg_sum[which("Pond" == sg_sum$Site),],aes(Year, X/area, colour = Site)) +
  geom_line() +
  geom_point() +
...
```{r Figure 2 remove pond}
ggplot(sg_sum[grep(paste(c("Pond"), collapse="|"), sg_sum$Site, invert=TRUE),],
       aes(Year, X/area, colour = Site)) +
  geom_line() +
  geom_point() +
  facet_wrap(~NS + SiteSame, ncol = 3) +
  ylab("Individuals per square meter") +
  theme(axis.text.x = element_text(angle = -90, vjust = .5, hjust = 1),
        panel.background = element_blank()) +
  scale_x_continuous(breaks=pretty_breaks(n=7))+
  scale_color_manual(values = c("#C67171","#C5C1AA","#8E8E38",
                                "#71C671","#7D9EC0","#7171C6",
                                "#C67171","#C5C1AA","#8E8E38",
                                "#71C671","#7D9EC0","#8B0000"))
...
test lapply below
```{r}
x<-sg_sum$Year[sg_sum$Site == "Atwell Gulch"]
y<-sg_sum$X[sg_sum$Site == "Atwell Gulch"]
...
apply(tlast.fit[tlast.fit$RangeEnd == 2015,4:6], 2, 
      FUN = function(x) mean(exp(as.numeric(x)), na.rm=TRUE)) #arithmetic mean

fit.2015 <- tlast.fit[tlast.fit$RangeEnd == 2015,]

fit.2015 <- transform(fit.2015, fit = exp(as.numeric(fit)), 
                      lwr = exp(as.numeric(lwr)), 
                      upr = exp(as.numeric(upr)))
fit.2015 <- data.frame(Site = rownames(fit.2015), fit.2015)
plot.2015 <- melt(fit.2015,
                  id.vars = "Site",
...
## Figure 4: Growth relationships between years for width (top) and height (bottom) for Sclerocactus glaucus individuals within demographic monitoring sites.    
