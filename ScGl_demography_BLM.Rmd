...
data.r <- cor.data[paste(cor.data.r$Var1, cor.data.r$Var2, sep="_") %in% combins.r,]
...
```{r}
ggplot(sg.pc.r, aes(Comp.1,Comp.2, colour=Site))+
  geom_point() +
  stat_ellipse() +
  theme_bw() 


ggplot(sg.pc.r, aes(Comp.1,Comp.2, colour=NS))+
  geom_point() +
  stat_ellipse() +
  theme_bw() 
...
sg.thsd <- TukeyHSD(aov(Comp.2~Site, data=sg.pc.r))
lasign <- multcompLetters(extract_p(sg.thsd$Site))
...
ggplot(sg.pc.r, aes(Site, Comp.2))+
  geom_boxplot()+
