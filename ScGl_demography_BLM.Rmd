...
get_prism_monthlys(type="tmax", mon = 1:12, keepZip = FALSE, years = 1986:currentYr)

# Minimum temperature (average)
get_prism_monthlys(type="tmin", mon = 1:12, keepZip = FALSE, years = 1986:currentYr)
```
...
```{r}
6cor.table<-cor(cc.season[,-c(1:2,grep("Year", names(cc.season)),
...
#### Figure 3: The Principal Components Analysis (PCA) of climatic variables with loadings  by a) genetic region and b) site. c) Site distribution along PC1 is shown with Tukey Honest Significant Differences. 
```{r}
ggplot(sg.pc, aes(Comp.1,Comp.2, colour=NS))+
  geom_point() +
  stat_ellipse() +
...
```{r}
recent <- season[season$Prev12 > 2007 & season$Prev12 < currentYr,]
...
```{r}
#doesn't like "-" in the name
levels(sg.pc$Site) <- c(as.character(unique(sg.pc$Site))[-(13:14)],"RoadTWest","TJunction")
