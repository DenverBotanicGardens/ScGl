...
sg <- read.csv(path.expand("P:/hackathon/ScGl/RawData_scgl_2015_103015.csv"))

head(sg)
names(sg)

table(sg$Site,sg$Year) #Are all the data lacking site information from Road T-East?
...
```{r}
# This is repetative from at_old and at_new
# Rename Sites for Atwell old and new

# Add a level to the Site factor field
levels(sg$Site) <- c(levels(sg$Site), "Atwell Gulch", "Atwell Gulch (old)")
...
# don't want to do this, want to do after combine again with sg so we have old and new Atwell
...
ggplot(atwell_summary, aes(Year,X,colour=Site))+
  geom_line()
```
...
str(sg_all)
#Reset to factor remaining Sites
sg_all$Site <- factor(sg_all$Site)
...
#Missing half of Pond data, need to remove for 2015
sg_sum <- sg_summary[-which(grepl("Pond", sg_summary$Site)&sg_summary$Year == 2015) ,]
